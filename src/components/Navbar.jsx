...
const Navbar = () => {
  ...
  const [isTop, setIsTop] = useState(true); // Track if the user is at the top
  const [lastScrollY, setLastScrollY] = useState(0); // Track last scroll position
  const [atTop, setAtTop] = useState(false); // Track if at the top of the website

  // Effect to track scroll position and update navbar visibility
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY === 0) {
        // If at the top of the page
        setIsTop(true);
        setAtTop(true); // Indicate that we are at the top of the page
      } else {
        setIsTop(false); // Remove top state
        setAtTop(false); // We're not at the top anymore
      }
  ...
  return (
    <>
      <motion.div
        initial={{ opacity: 1, y: 0 }} // Initial state (visible, in place)
        animate={{
          opacity: isVisible ? 1 : 0, // Fade in/out based on visibility
          y: isVisible ? 0 : -80, // Move navbar up when hidden
          transition: { duration: 0.3, ease: "easeOut" },
        }}
        className={`fixed z-[999] w-full px-20 py-8 font-['Neue_Montreal'] flex justify-between items-center ${
          isTop
            ? "bg-transparent shadow-none" // When at the top, no background and no shadow
            : "bg-white/30 backdrop-blur-lg shadow-lg" // Apply glassmorphic effect when scrolling down
        }`}
      >
        <div className="logo">
          <svg
        ...
        <div className="links flex gap-10">
          {["Services", "Our Work", "About Us", "Insights", "Contact"].map(
            (item, index) => (
              <a
                key={index}
                className={`capitalize text-lg font-regular ${
                  index === 4 && "ml-32"
                }`}
                href="#"
              >
